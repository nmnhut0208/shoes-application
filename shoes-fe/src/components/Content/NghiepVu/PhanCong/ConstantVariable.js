import { rem_to_px } from "~config/ui";

export const INFO_COLS_DONHANG = [
  { header: "Số đơn hàng", key: "SODH", width: 21 * rem_to_px },
  { header: "Ngày đơn hàng", key: "NGAYDH", width: 16 * rem_to_px },
  { header: "Mã khách hàng", key: "MAKH", width: 16 * rem_to_px },
  { header: "Tên khách hàng", key: "TENKH", width: 25 * rem_to_px },
  { header: "Diễn dãi", key: "DIENGIAI", width: 35 * rem_to_px },
  { header: "Tổng số lượng đặt hàng", key: "SOLUONG", width: 21 * rem_to_px },
];

export const INFO_COLS_CHITIET_PHANCONG = [
  { header: "Số đơn hàng", key: "SODH", width: 15 * rem_to_px },
  { header: "Mã giày", key: "MAGIAY", width: 20 * rem_to_px },
  { header: "Màu đế", key: "TENMAUDE", width: 10 * rem_to_px },
  { header: "Màu gót", key: "TENMAUGOT", width: 10 * rem_to_px },
  { header: "Màu sườn", key: "TENMAUSUON", width: 10 * rem_to_px },
  { header: "Màu cá", key: "TENMAUCA", width: 10 * rem_to_px },
  { header: "Màu quai", key: "TENMAUQUAI", width: 10 * rem_to_px },
  { header: "Thợ đế", key: "THODE", width: 15 * rem_to_px },
  { header: "Thợ quai", key: "THOQUAI", width: 15 * rem_to_px },
  { header: "Size 5", key: "SIZE5", width: 7 * rem_to_px },
  { header: "Size 6", key: "SIZE6", width: 7 * rem_to_px },
  { header: "Size 7", key: "SIZE7", width: 7 * rem_to_px },
  { header: "Size 8", key: "SIZE8", width: 7 * rem_to_px },
  { header: "Size 9", key: "SIZE9", width: 7 * rem_to_px },
  { header: "Size 0", key: "SIZE0", width: 7 * rem_to_px },
  { header: "Tên giày", key: "TENGIAY", width: 15 * rem_to_px },
];
